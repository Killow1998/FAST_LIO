...
public:
  ...
  // modify by h2q
  void IMUstateInit(rclcpp::Node::SharedPtr nh, std::string frame_id,
                    std::string child_id, std::string topic, double grav_val);
...
private:
  ...
  // modify by h2q
  nav_msgs::msg::Odometry odomfromIMU_;
  string odomfromIMU_frame_;
  string odomfromIMU_child_frame_;
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr odomfromIMU_pub_;
...
void ImuProcess::IMU_init(
  ...
  /* --- modify by h2q use gravity_m_s2_ to align gravity --- */
  // init_state.grav = S2(-mean_acc / mean_acc.norm() * G_m_s2);

  V3D meas_grav_in_body = mean_acc;
  V3D target_grav_in_world(0, 0, 1.0);

  Eigen::Quaterniond q_init;
  q_init.setFromTwoVectors(meas_grav_in_body.normalized(),
                           target_grav_in_world);
...
void ImuProcess::UndistortPcl(
  ...
  double imu_beg_time = rclcpp::Time(v_imu.front()->header.stamp).seconds();
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    odomfromIMU_.child_frame_id = odomfromIMU_child_frame_;
    odomfromIMU_.pose.pose.position.x = imu_state.pos(0);
    odomfromIMU_.pose.pose.position.y = imu_state.pos(1);
    odomfromIMU_.pose.pose.position.z = imu_state.pos(2);
    odomfromIMU_.pose.pose.orientation.x = q_curr.x();
    odomfromIMU_.pose.pose.orientation.y = q_curr.y();
    odomfromIMU_.pose.pose.orientation.z = q_curr.z();
    odomfromIMU_.pose.pose.orientation.w = q_curr.w();
    odomfromIMU_.twist.twist.linear.x = imu_state.vel(0);
    odomfromIMU_.twist.twist.linear.y = imu_state.vel(1);
    odomfromIMU_.twist.twist.linear.z = imu_state.vel(2);
...
// modify by h2q
void ImuProcess::IMUstateInit(rclcpp::Node::SharedPtr nh, std::string frame_id,
  ...
  odomfromIMU_pub_ = nh->create_publisher<nav_msgs::msg::Odometry>(topic, 1000);

  /* Initialize the Odometry message structure with default/static values */
  odomfromIMU_.header.frame_id = odomfromIMU_frame_;
  odomfromIMU_.child_frame_id = odomfromIMU_child_frame_;

  // Initialize orientation to identity quaternion to prevent invalid data
  // initially
  odomfromIMU_.pose.pose.position.x = 0.0;
  odomfromIMU_.pose.pose.position.y = 0.0;
  odomfromIMU_.pose.pose.position.z = 0.0;
  odomfromIMU_.pose.pose.orientation.w = 1.0;
  odomfromIMU_.pose.pose.orientation.x = 0.0;
  odomfromIMU_.pose.pose.orientation.y = 0.0;
  odomfromIMU_.pose.pose.orientation.z = 0.0;
}
